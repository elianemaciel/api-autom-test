...
class CheckInsertedDataWidget:
    position = None
    instance = None

    @staticmethod
    def get_or_start(position=None):
        if CheckInsertedDataWidget.position is None and position is None:
            raise ValueError("InsertMethodsInfo not started yet. position has value 'None'. Please start the page first")
        if CheckInsertedDataWidget.position is not None:
            return CheckInsertedDataWidget.instance
        CheckInsertedDataWidget.position = position
        CheckInsertedDataWidget.instance = CheckInsertedDataWidget._setup()
        return CheckInsertedDataWidget.instance
    ...
    def _setup():
        ...
        program_description.setText("Content for Check Inserted Data Info page")
